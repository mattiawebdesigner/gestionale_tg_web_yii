...
<span class="btn btn-info" data-btn-copy data-type="entrata"><i class="fas fa-plus"></i> <?= Yii::t('app', 'Aggiungi') ?></span>
...
<table data-upload class="table">
    ...
    <?php foreach($in as $key => $value) : ?>
        <tr data-action-panel class="action-panel">
            <td><span data-type="entrata" data-id="<?= $value->id ?>" data-save><?= Yii::t('app', 'Salva') ?></span></td>
            <td><span data-type="entrata" data-id="<?= $value->id ?>" data-delete><?= Yii::t('app', 'Cancella') ?></span></td>
